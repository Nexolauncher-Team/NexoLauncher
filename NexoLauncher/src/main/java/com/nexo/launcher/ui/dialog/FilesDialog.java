...
import com.nexo.launcher.databinding.DialogOperationFileBinding;
...
public class FilesDialog extends FullScreenDialog implements DraggableDialog.DialogInitializationListener {
    ...
    private void handleButtons(FilesButton filesButton, Task<?> endTask) {
        ...
        binding.copyView.setOnClickListener(v -> {
            if (this.mCopyClick != null) {
                pasteFile.setPaste(mRoot, mSelectedFiles, PasteFile.PasteType.COPY); // å¤åˆ¶æ¨¡å¼
        ...
        binding.moveView.setOnClickListener(v -> {
            if (this.mCopyClick != null) {
                pasteFile.setPaste(mRoot, mSelectedFiles, PasteFile.PasteType.MOVE); // ç§»åŠ¨æ¨¡å¼
                this.mCopyClick.onButtonClick();
            }
            closeDialog();
        });

        if (mSelectedFiles.size() == 1) { //å•é€‰æ¨¡å¼
            ...
            //å¤šé€‰æ¨¡å¼ç¦æ­¢ä½¿ç”¨åˆ†äº«ã€é‡å‘½å
    ...
    //æ­¤æ–¹æ³•è¦åœ¨è®¾ç½®ç‚¹å‡»äº‹ä»¶ä¹‹åŽè°ƒç”¨ï¼Œå¦åˆ™ç¦ç”¨æŒ‰é’®åŽæŒ‰é’®ä»ç„¶èƒ½å¤Ÿç‚¹å‡»
