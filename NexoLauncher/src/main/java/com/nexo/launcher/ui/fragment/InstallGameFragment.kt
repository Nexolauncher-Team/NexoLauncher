...
import com.nexo.launcher.databinding.FragmentInstallGameBinding
...
import com.nexo.launcher.feature.version.install.InstallArgsUtils
...
import com.nexo.launcher.ui.fragment.download.addon.DownloadFabricApiFragment
import com.nexo.launcher.ui.fragment.download.addon.DownloadFabricFragment
import com.nexo.launcher.ui.fragment.download.addon.DownloadForgeFragment
import com.nexo.launcher.ui.fragment.download.addon.DownloadNeoForgeFragment
import com.nexo.launcher.ui.fragment.download.addon.DownloadOptiFineFragment
import com.nexo.launcher.ui.fragment.download.addon.DownloadQuiltApiFragment
import com.nexo.launcher.ui.fragment.download.addon.DownloadQuiltFragment
...
class InstallGameFragment : FragmentWithAnim(R.layout.fragment_install_game), View.OnClickListener {
    ...
    /**
     * æ£€æŸ¥ä¸å…¼å®¹çš„Addonï¼Œå¹¶ç¦æ­¢ç”¨æˆ·é€‰æ‹©è¯¥Addonç‰ˆæœ¬
     */
    ...
    /**
     * æ£€æŸ¥ä¼ å…¥çš„Addonæ˜¯å¦åœ¨AddonMapä¸­æœ‰ä¸å…¼å®¹çš„Addon
     * @param addon ä¼ å…¥çš„Addon
     * @param layout Addonçš„layout
     * @param versionText Addonçš„ç‰ˆæœ¬ä¿¡æ¯
     * @param installText Addonçš„å®‰è£…ç±»åž‹
     */
    private fun checkIncompatible(
        addon: Addon,
        layout: View,
    ...
    /**
     * ç§»é™¤Addonï¼Œå¹¶åˆ·æ–°å½“å‰ä¸å…¼å®¹çš„Addon
     */
        ...
        binding.apply {
            when (v) {
                ...
                install -> {
                    ...
                    //æ£€æŸ¥OptiFineä¸ŽForgeé™„åŠ åŒ…æ˜¯å¦åŒæ—¶å­˜åœ¨
                    //æœ€åŽå‘Šè¯‰ç”¨æˆ·å…¼å®¹æ€§é—®é¢˜
    ...
    private fun organizeInstallationTasks(customVersionName: String): Map<Addon, InstallTaskItem> {
        ...
        fun getModPath(): File {
            return if (AllSettings.versionIsolation.getValue()) //å¯ç”¨äº†ç‰ˆæœ¬éš”ç¦»
                File(
        ...
        addonMap.forEach { (addon, taskPair) ->
            when (addon) {
                Addon.OPTIFINE -> {
                    val endTask: InstallTaskItem.EndTask = if (mapSize < 2) { //å®‰è£…ä¸ºä¸€ä¸ªç‰ˆæœ¬
    ...
    /**
     * åœ¨JavaGUIå†…è¿›è¡Œå®‰è£…ï¼Œä½œä¸ºEndTaskï¼Œéœ€è¦åœ¨UIçº¿ç¨‹å†…è¿è¡Œ
     * @param activity **æ­¤å¤„å¿…é¡»ä½¿ç”¨activityçš„ä¸Šä¸‹æ–‡ï¼ä¸èƒ½è°ƒç”¨Fragmentçš„ä¸Šä¸‹æ–‡ï¼ï¼å› ä¸ºè°ƒç”¨åˆ°è¿™é‡Œçš„æ—¶å€™ï¼ŒFragmentæ—©å°±è¢«é”€æ¯äº†ï¼ï¼ï¼**
     */
    @Throws(Throwable::class)
