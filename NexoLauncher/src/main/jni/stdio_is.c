...
Java_com_nexo_launcher_Logger_begin(JNIEnv *env, __attribute((unused)) jclass clazz, jstring logPath) {
    if (latestlog_fd != -1)
    {
    ...
    {
        jclass eventLogListener = (*env)->FindClass(env, "com/nexo/launcher/Logger$eventLogListener");
...
JNIEXPORT void JNICALL Java_com_nexo_launcher_Logger_appendToLog(JNIEnv *env, __attribute((unused)) jclass clazz, jstring text) {
...
JNIEXPORT void JNICALL
Java_com_nexo_launcher_Logger_setLogListener(JNIEnv *env, __attribute((unused)) jclass clazz, jobject log_listener) {
...
JNIEXPORT void JNICALL
Java_com_nexo_launcher_utils_JREUtils_setupExitMethod(JNIEnv *env, jclass clazz,
                                                        jobject context) {
    exitTrap_ctx = (*env)->NewGlobalRef(env,context);
    (*env)->GetJavaVM(env,&exitTrap_jvm);
    exitTrap_exitClass = (*env)->NewGlobalRef(env,(*env)->FindClass(env,"com/nexo/launcher/ui/activity/ErrorActivity"));
