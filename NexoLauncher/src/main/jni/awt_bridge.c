...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    if (dalvikJavaVMPtr == NULL) {
        ...
        class_MainActivity = (*env)->NewGlobalRef(env,(*env)->FindClass(env, "com/nexo/launcher/MainActivity"));
...
JNIEXPORT void JNICALL Java_com_nexo_launcher_AWTInputBridge_nativeSendData(JNIEnv* env, jclass clazz, jint type, jint i1, jint i2, jint i3, jint i4) {
...
JNIEXPORT jintArray JNICALL Java_com_nexo_launcher_utils_JREUtils_renderAWTScreenFrame(JNIEnv* env, jclass clazz /*, jobject canvas, jint width, jint height */) {
...
JNIEXPORT void JNICALL Java_com_nexo_launcher_AWTInputBridge_nativeClipboardReceived(JNIEnv *env, jclass clazz, jstring clipboardData, jstring clipboardDataMime) {
...
Java_com_nexo_launcher_AWTInputBridge_nativeMoveWindow(JNIEnv *env, jclass clazz, jint xoff, jint yoff) {
    if (runtimeJNIEnvPtr_INPUT == NULL) {
        if (runtimeJavaVMPtr == NULL) {
            return;
        } else {
